...
import loo2.plp.orientadaObjetos1.comando.ReadFromBlockchain;
import loo2.plp.orientadaObjetos1.comando.WriteToBlockchain;
import loo2.plp.orientadaObjetos1.comando.TransferAtBlockchain;
...
import loo2.plp.orientadaObjetos2.declaracao.classe.DecClasseContratoSimplesOO2;
import loo2.plp.orientadaObjetos1.declaracao.procedimento.DecProcedimentoPagavelSimples;
...
public class OO2Parser implements OO2ParserConstants {
  ...
  public static void main(String args [])
  {
    ...
    try
    {
      ...
      if (programa.checaTipo(new ContextoCompilacaoOO2(new ListaValor(), null)))
      {
        programa.executar(new ContextoExecucaoOO2(), null);
      }
      else
      {
...
/** * Adicionando as opções de escrita na blockchain e leitura da blockchain */
  ...
  IO retorno;
    if (jj_2_18(2147483647)) {
      ...
      retorno = PReadBlockchain();
    } else if (jj_2_21(2147483647)) {
      retorno = PWriteToBlockchain();
    } else if (jj_2_22(2147483647)) {
      retorno = PTransferAtBlockchain();
    } else {
      jj_consume_token(-1);
      throw new ParseException();
    }
...
/** * HFA  "lerDaBlockChain" "(" PInteiro ")" **/
  final public ReadFromBlockchain PReadBlockchain() throws ParseException {
  Expressao endereco;
    jj_consume_token(READBLOCKCHAIN);
    jj_consume_token(LPAREN);
    endereco = PExpressao();
    jj_consume_token(RPAREN);
    {if (true) return new ReadFromBlockchain(endereco);}
    throw new Error("Missing return statement in function");
  }
...
/** * HFA  "escreverNaBlockchain" "("  ")" **/
  final public WriteToBlockchain PWriteToBlockchain() throws ParseException {
  Expressao endereco;
  Expressao saldo;
    jj_consume_token(WRITEBLOCKCHAIN);
    jj_consume_token(LPAREN);
    endereco = PExpressao();
    jj_consume_token(COMMA);
    saldo = PExpressao();
    jj_consume_token(RPAREN);
    {if (true) return new WriteToBlockchain(endereco, saldo);}
    throw new Error("Missing return statement in function");
  }

/** * HFA  "transferir" "(" origem, destino, valor ")" **/
  final public TransferAtBlockchain PTransferAtBlockchain() throws ParseException {
  Expressao enderecoOrigem;
  Expressao enderecoDestino;
  Expressao valor;
    jj_consume_token(TRANSFERIR);
    jj_consume_token(LPAREN);
    enderecoOrigem = PExpressao();
    jj_consume_token(COMMA);
    enderecoDestino = PExpressao();
    jj_consume_token(COMMA);
    valor = PExpressao();
    jj_consume_token(RPAREN);
    {if (true) return new TransferAtBlockchain(enderecoOrigem, enderecoDestino, valor);}
    throw new Error("Missing return statement in function");
  }
...
/** * HFA - Adicionei mais um lookahead para contemplar as classes do tipo contrato. No código eu criei uma nova classe que herda da declaração simples. */
  ...
  DecClasse resposta;
    if (jj_2_45(2147483647)) {
      ...
      /** HFA
       * Aqui parece ser o lugar para adicionar os atributos implícitos dos contratos.
       */
            
		Tipo tipoEnd = TipoPrimitivo.TIPO_INTEIRO;
		Id idAtributoEnd = new Id("endereco");
		ValorInteiro enderecoNaBlockchain = new ValorInteiro(1212);

		Tipo tipoSaldo = TipoPrimitivo.TIPO_INTEIRO;
		Id idAtributoSaldo = new Id("saldo");
		ValorInteiro saldoDoContrato = new ValorInteiro(100);

		SimplesDecVariavel atributoEndereco = new SimplesDecVariavel(tipoEnd, idAtributoEnd, enderecoNaBlockchain);
		SimplesDecVariavel atributoSaldo = new SimplesDecVariavel(tipoSaldo, idAtributoSaldo, saldoDoContrato);

		if (atributos instanceof SimplesDecVariavel) {
			CompostaDecVariavel decCompostaImp = new CompostaDecVariavel(atributoEndereco, atributoSaldo);
			CompostaDecVariavel decCompostaNova = new CompostaDecVariavel(atributos, decCompostaImp);
			atributos = decCompostaNova;
		} else {
			DecVariavel atributosAux = atributos;

			while (true) {
				if (atributosAux instanceof CompostaDecVariavel
						&& ((CompostaDecVariavel) atributosAux).getDeclaracao2() instanceof CompostaDecVariavel) {
					atributosAux = ((CompostaDecVariavel) atributosAux).getDeclaracao2();
					continue;
				} else {
					CompostaDecVariavel decCompostaImp = new CompostaDecVariavel(atributoEndereco, atributoSaldo);
					CompostaDecVariavel decCompostaNova = new CompostaDecVariavel(((CompostaDecVariavel) atributosAux).getDeclaracao2(), decCompostaImp);
					((CompostaDecVariavel) atributosAux).setDeclaracao2(decCompostaNova);
					break;
				}
			}
		}		
      ...
      //HFA - Adicionaria o método
      
      jj_consume_token(RBRACE);
                                resposta = new DecClasseContratoSimplesOO2(nomeClasse, superClasse, atributos, decConstrutor, metodos);
    } else {
      jj_consume_token(-1);
      throw new ParseException();
    }
...
/** * HFA - Adicionei o modificador pagavel */
  ...
  boolean eContrato = false;
    ...
    if (eContrato) {
      {if (true) return new DecProcedimentoPagavelSimples(nome, listaPar, comando);}
    } else {
...
/** * TipoClasse ::= Id **/
  ...
  private boolean jj_3R_115() {
    if (jj_scan_token(TRANSFERIR)) return true;
    if (jj_scan_token(LPAREN)) return true;
    if (jj_3R_8()) return true;
    if (jj_scan_token(COMMA)) return true;
    if (jj_3R_8()) return true;
  ...
  private boolean jj_3R_114() {
    if (jj_scan_token(WRITEBLOCKCHAIN)) return true;
    if (jj_scan_token(LPAREN)) return true;
    if (jj_3R_8()) return true;
    if (jj_scan_token(COMMA)) return true;
    if (jj_3R_8()) return true;
  ...
  private boolean jj_3_58() {
    ...
    if (jj_scan_token(PAGAVEL)) return true;
    if (jj_3R_6()) return true;
    if (jj_scan_token(LPAREN)) return true;
    if (jj_3R_20()) return true;
    if (jj_scan_token(RPAREN)) return true;
  ...
  private boolean jj_3R_113() {
    if (jj_scan_token(READBLOCKCHAIN)) return true;
    if (jj_scan_token(LPAREN)) return true;
    if (jj_3R_8()) return true;
    if (jj_scan_token(RPAREN)) return true;
    return false;
  ...
  private boolean jj_3_47() {
    ...
    if (jj_scan_token(CONTRATO)) return true;
    if (jj_3R_6()) return true;
    return false;
  }
  ...
  static {
   ...
   private static void jj_la1_init_0() {
      jj_la1_0 = new int[] {0x23b0000,0x0,0x200000,0x2000000,0xa007800,0x2000000,0x8006000,0x0,0x2000,0xa007800,0x0,0x400,0x400,0x200,0x0,0x2000000,0x0,0x4000000,0x0,0x0,0x0,0x10000000,0x0,0x0,0x0,};
   }
   private static void jj_la1_init_1() {
      jj_la1_1 = new int[] {0x14807,0x4000,0x0,0x4800,0x4c40,0x4800,0x440,0x400,0x0,0x4c40,0x80000000,0x0,0x0,0x0,0x4800,0x0,0x800,0x0,0x4838,0x800,0x838,0x0,0x838,0x800,0x20,};
   }
   private static void jj_la1_init_2() {
      jj_la1_2 = new int[] {0x0,0x0,0x0,0x0,0x80,0x0,0x0,0x0,0x0,0x80,0x0,0xe0,0xe0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,};
   }
